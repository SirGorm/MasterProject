"""
Centralized Configuration Module for Strength Training ML Pipeline (v2).
...
Changes from v1:
- Identical configuration, updated PROJECT_ROOT for v2 folder
"""
...
class DataConfig:
    ...
    exercises: List[str] = field(default_factory=lambda: ['Squat', 'Benchpress', 'Deadlift'])
...
class TrainingConfig:
    ...
    loss_weights: Dict[str, float] = field(default_factory=lambda: {
        'exercise': 1.0, 'reps': 0.5, 'phase': 1.0, 'fatigue': 1.0
    })
    device: str = 'auto'
...
class EvaluationConfig:
    ...
    plot_types: List[str] = field(default_factory=lambda: [
        'training_history', 'confusion_matrix', 'phase_confusion_matrix',
        'rep_counting_analysis', 'fatigue_regression', 'attention_weights'
    ])
...
class FatigueConfig:
    ...
    emg_window_samples: int = 1000
    continuous_output: bool = True
    n_fatigue_levels: int = 3
    thresholds: Dict[str, float] = field(default_factory=lambda: {
        'low_moderate': 0.33, 'moderate_high': 0.67
    })
...
class MLflowConfig:
    ...
    experiment_name: str = "strength_training_v3"
...
class Config:
    ...
    def get_active_exercises(self) -> List[str]:
        """Return exercises minus excluded ones."""
        return [e for e in self.data.exercises if e not in self.data.excluded_exercises]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        def convert(obj):
            if isinstance(obj, Path):
                ...
                return {
                    'name': obj.name, 'sampling_rate': obj.sampling_rate,
                    'channels': obj.channels, 'file_name': obj.file_name,
                    'enabled': obj.enabled
                }
            return obj
    ...
    def print_summary(self):
        print("\n" + "="*70)
        print("CONFIGURATION SUMMARY (v2 - Lightning)")
        print("="*70)
        print(f"\nDataset: {self.data.dataset_path}")
        active = self.get_active_exercises()
        print(f"Exercises: {', '.join(active)}")
        if self.data.excluded_exercises:
            print(f"Excluded:  {', '.join(self.data.excluded_exercises)}")
