...
class StrengthTrainingModel(nn.Module):
    ...
    def __init__(self, config=None):
        ...
        n_exercises = len(self.config.get_active_exercises()) if hasattr(self.config, 'get_active_exercises') \
            else len(self.config.data.exercises)
    ...
    def fuse_for_task(
        ...
        if not embeddings:
            # No signals available - this should not happen in normal operation
            # Try to infer batch_size from encoded_signals if any exist
            if encoded_signals:
                first_signal = next(iter(encoded_signals.values()))
                batch_size = first_signal.size(0)
            else:
                # Last resort: assume batch_size=1 and log warning
                import warnings
                warnings.warn(f"No signals available for task '{task_name}'. Using batch_size=1.")
                batch_size = 1
...
class MultiTaskLoss(nn.Module):
    ...
    Rep counting uses BCEWithLogitsLoss for binary classification.
    """
    ...
    def __init__(
        ...
        # Binary classification for reps (0 or 1 per window) with class imbalance handling
        self.rep_criterion = nn.BCEWithLogitsLoss(
            pos_weight=torch.tensor([rep_pos_weight])
        )
