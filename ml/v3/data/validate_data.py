...
class DataValidator:
    ...
    def _discover_structure(self):
        ...
        Supports both flat (Exercise/1/, Exercise/2/) and nested
        (Exercise/PersonXX/SetYY/) directory layouts.
        """
        print(f"\nScanning dataset: {self.dataset_path}")

        active_exercises = self.config.get_active_exercises() if hasattr(self.config, 'get_active_exercises') \
            else [e for e in self.config.data.exercises if e not in getattr(self.config.data, 'excluded_exercises', [])]

        for exercise_name in active_exercises:
            ...
            session_dirs = self._find_session_dirs(exercise_path)
    ...
    def _find_session_dirs(exercise_path: Path) -> List[Tuple[str, Path]]:
        ...
        for child in sorted(exercise_path.iterdir()):
            ...
            if has_data:
                # Flat layout: Exercise/SessionDir/
